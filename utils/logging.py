# This code is a modified version of loggers.py by Cog-Creators
    ...
    *started* with the backup number incremented. The stdlib handler
    ...
    def doRollover(self):
        ...
        if self.backupCount < 1:
            ...
            # bot-part2.log becomes bot-part1.log etc, a new log is added at the end.
...
def init_loggers(level: int, location: pathlib.Path) -> None:
    """
    Initialize the logging system for ATOS.

    Arguments
    ---------
    level: int
        Level of the logger, from DEBUG (10) to CRITICAL (50)
    ...
    base_logger = logging.getLogger("atos")
    ...
    latest_fhandler = RotatingFileHandler(
        stem="latest",
        directory=location,
        maxBytes=1000000,  # About 1MB per logfile
