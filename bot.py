...
async def on_command_error(ctx, error):
    if isinstance(error, (commands.CheckFailure, commands.MissingRole, commands.NotOwner)):
        ...
        await ctx.message.add_reaction("💿")
        await ctx.send(f"<@{ctx.author.id}> Les paramètres de cette commande sont mal renseignés. Utilise `{bot_prefix}help` pour en savoir plus.")
        ...
        await ctx.message.add_reaction("❔")
        await ctx.send(f"<@{ctx.author.id}> Voulais-tu écrire autre chose ? Utilise `{bot_prefix}help` pour avoir la liste des commandes.")
